...
function podcast_js_scripts() {
	...
	if (! is_admin()) {
		...
		wp_register_script( 'podcast-scripts', get_template_directory_uri() . '/js/podcast.js', array( 'jquery' ), '20180502', true );
...
function podcast_wp_title( $title, $sep ) {
	...
	if ( $paged >= 2 || $page >= 2 ) {
		$title = "$title $sep " . sprintf( esc_html__( 'Page %s', 'podcast' ), max( $paged, $page ) );
	}
				...
				<div class="comment-metadata">
					<a href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>">
						<time datetime="<?php comment_time( 'c' ); ?>">
							<?php printf( esc_html_x( '%1$s at %2$s', '1: date, 2: time', 'podcast' ), get_comment_date(), get_comment_time() ); ?>
						</time>
					</a>
